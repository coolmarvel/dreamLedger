...
import { block_manager_servers } from "../lib/api/auth";
...
const INITIALIZE_FORM2 = "auth/INITIALIZE_FORM2";

const [
  BLOCK_MANAGER_SERVERS,
  BLOCK_MANAGER_SERVERS_SUCCESS,
  BLOCK_MANAGER_SERVERS_FAILURE,
] = createRequestActionTypes("auth/BLOCK_MANAGER_SERVERS");

const [LOGIN, LOGIN_SUCCESS, LOGIN_FAILURE] =
  createRequestActionTypes("auth/LOGIN");

const [UPDATEPWD, UPDATEPWD_SUCCESS, UPDATEPWD_FAILURE] =
  createRequestActionTypes("auth/UPDATEPWD");

const [DOWNLOAD_FILE, DOWNLOAD_FILE_SUCCESS, DOWNLOAD_FILE_FAILURE] =
  createRequestActionTypes("user/DOWNLOAD_FILE");

const [LOGOUT, LOGOUT_SUCCESS, LOGOUT_FAILURE] =
  createRequestActionTypes("auth/LOGOUT");
const [SAVE, SAVE_SUCCESS, SAVE_FAILURE] =
  createRequestActionTypes("auth/SAVE");
...
export const addSubField = createAction(ADD_SUB_FIELD, ({ form, key }) => ({
  form, // register , login
  key, // username, password, passwordConfirm
}));
...
export const initializeForm2 = createAction(
  INITIALIZE_FORM2,
  ({ form, sub }) => ({ form, sub })
); // register / login
export const blockManagerServers = createAction(
  BLOCK_MANAGER_SERVERS,
  (data) => data
);
...
// saga 생성
const blockManagerServersSaga = createRequestSaga(
  BLOCK_MANAGER_SERVERS,
  authAPI.block_manager_servers
);
...
const auth = handleActions(
  {
    ...
    // 회원가입 성공
    [BLOCK_MANAGER_SERVERS_SUCCESS]: (state, { payload: data }) =>
      produce(state, (draft) => {
    ...
    // 회원가입 실패
    [BLOCK_MANAGER_SERVERS_FAILURE]: (state, { payload: error }) =>
      produce(state, (draft) => {
    ...
    [CHANGE_FILE_FIELD]: (
      state,
      { payload: { form, fileId, key, seq, fieldId, value } }
    ) =>
      produce(state, (draft) => {
        const index = draft[form].findIndex(
          (element) => fileId === undefined && element.seq === seq
        );
        if (index !== -1) {
    ...
    [SAVE_SUCCESS]: (state, { payload: result }) =>
      produce(state, (draft) => {
