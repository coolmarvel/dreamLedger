...
const  StudentInfoField = (props) => {
    ...
    const { member,student,onChange ,onSubChange,editing,onChangeImgFile,onSave} = props;
    const { m_studentFile,address,zipcode,detailAddress,baseUrl} = useSelector(({ auth}) => ({
    ...
    const handleChange = e => {
        ...
        if(file.id !== undefined){
            onChangeImgFile(file.id,file.key,student.seq,student.fieldId,e.target.files[0]);
        }
        else {
            let fileObj = new Object();
            fileObj.key = getUUID();
            setFile(fileObj);
            onChangeImgFile(fileObj.id,fileObj.key,student.seq,student.id,e.target.files[0]);
        }
...
const onPreview = () => {
    ...
    if (editing) { // 수정모드
                ...
                <div className="{classes.root} edit-cont-wrap" >
                    ...
                    <div className="photo-edit-cont">
                        ...
                        <div className="cont-left" >
                            ...
                            <TextField   className="itxt-wrap01 cell01-05" name="phoneNumber" label="Outlined"
                            ...
                            <div style={{display:'inline'}}>
                                <TextField   className="itxt-wrap01 cell01-07" name="address" label="Outlined"  variant="outlined" label={"주소"}  value={student.address} onClick={popupPost}/>
                            </div>
            ...
            return(
                <div className="{classes.root} edit-cont-wrap">
                    <h1 className="main-tit01">나에 대해 알려줍시다. <br className="mob-inline"/>강점, 목표, 관심분야도 좋아요.</h1>
                    <div className="photo-cont">
                        <img src={state.image_preview}
                        alt="증명사진" className="id-img"/>

                        <div className="cont-left01">
                            <div className="name-wrap"><span className="name">{member.name}</span> {student.sex}</div>
                            <div className="info"><span className="info-tit">생년월일</span>{student.birthday}</div>
                            <div className="info"><span className="info-tit">휴대폰</span>{student.mobileNumber}</div>
                            <div className="info"><span className="info-tit">전화번호</span>{student.phoneNumber}</div>
                            <div className="info"><span className="info-tit">주소</span>{student.address}</div>
                            <div className="info"><span className="info-tit">이메일</span>{member.email}</div>
                        </div>
