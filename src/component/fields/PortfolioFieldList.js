...
const  PortfolioFieldList = (props) => {
    ...
    const { data,filePart,onUpdate,editing,onChangeImgFile,refresh,onAdd,onAddFile,onRemove,onRemoveFile} = props;
    ...
    return (
        <div>
            ...
            {editing && <div className="btn-add-wrap">
                <button type="button" className="btn-cont-add01" onClick={onAddFile}><span
                    className="plus">+</span><span className="add-txt">파일 추가</span></button>
                <button type="button" className="btn-cont-add01" onClick={onAdd}><span
                    className="plus">+</span><span className="add-txt">URL 추가</span></button>
            </div>}
